class Enemy {
  constructor(x, y, race, skill, minLvl, maxLvl, offSetX = 0, offSetY = 0, attack = EnemyAttack, wid = spriteSize.width, hei = spriteSize.height, fontSizeChange = fontSize.playersDisplay, deathImage = sprites.death) {
    ...
    // path finding
    this.dist = (width+height)*(this.agi/25);
  ...
  // AI
  movement(worldW, worldH, playerX, playerY, playerLvl) {
    // not stuned
    if (!this.stun) {
      // attack player
      if (this.onScreen()) {
  ...
  goToPoint(pointX = this.headToX, pointY = this.headToY) {
    ...
    if (this.x > pointXMax) { // left
      moved = true;
      this.x -= this.speed*0.70;
      this.mapX -= this.speed*0.70;
    }
    ...
    if (this.y > pointYMax) { // up
      moved = true;
      this.y -= this.speed*0.70;
      this.mapY -= this.speed*0.70;
    }
  ...
  attackPlayer() {
    if (this.attackState === 1) {
      ...
      this.allAttacks.push(new this.attackType(0, 0, this.x, this.y, this.speed*this.attackPattern.attackSpeed, this.attackDist, this.mainDmg, this.attackPattern.img, this.attackPattern.soundHit));
      this.attackState = 0;
