      ...
      if (guildInventory[y][xIndex] !== "empty") {
        if (allItems.get("Money").amount >= guildInventory[y][xIndex].cost) {
          ...
          if (guildInventory[y][xIndex].name === "Guild Ticket") {
            ...
            guildMembers.push(new GuildMember(guildInventory[y][xIndex].race, guildInventory[y][xIndex].skill, guildInventory[y][xIndex].stats.lvl));
      ...
      for (let x = 0; x < guildInventory[y].length; x++) {
        ...
        if (guildInventory[y][x] === "empty" && addOption) {
          ...
          if (itemSpawnChance >= guildInventorySize/10) {
            guildInventory[y][x] = {name: random(names), description: "A party member\nwho fights with you.", race: random(raceSpecific.guild), skill: random(skillSpecific.guild), stats: {lvl: constrain(int(random(player.lvl-5, player.lvl+3)), 0, Infinity)}, cost: 0};
