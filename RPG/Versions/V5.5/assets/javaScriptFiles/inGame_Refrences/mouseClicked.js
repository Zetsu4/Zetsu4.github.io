  ...
  if (startingState === 2 && state === 0) {
    if (!player.coolDown) {
      ...
      if (player.attackState === "melee") {
        items.playerAttack.push(new Attack(0, 0, 0, 0, melee.attackSpeed, melee.attackDist, player.mDmg, melee.img, sounds.swordHit));
        attacked = true;
      ...
      if (player.attackState === "magic" && player.mp >= 10) {
        player.mp -= 10;
        items.playerAttack.push(new Attack(0, 0, 0, 0, spellCaster.attackSpeed, spellCaster.attackDist, player.sDmg, spellCaster.img, sounds.fireballHit));
        attacked = true;
